...
def  main():
  ...
  installer = "\installer\Tenebris Installer.exe"
   
  if demo:
    installer = "\installer\Tenebris Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Tenebris manual.pdf" 
  ]
  ...
  ZIPNAME = "Tenebris-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Tenebris-v" + FULLVERSIONSTR + "-win-demo.zip"
